          ...
          <div className="relative" onMouseEnter={() => setNatOpen(true)} onMouseLeave={() => setNatOpen(false)}>
            ...
            {natOpen && (
              <div className="absolute top-full left-0 pt-2 w-64" data-testid="nav-natural-menu">
                <div className="glass border border-[#D9CB94] py-2">
                  {NAT_SUBNAV.map(n => (
                    <Link key={n.to} to={n.to} className={`block px-5 py-2.5 text-sm hover:bg-[#FFF8E1] transition ${n.pink ? 'text-[#B7536B]' : n.blue ? 'text-[#2F8FCB]' : 'text-[#1A1505]'}`} data-testid={`nav-sub-${n.label.replace(/\s+/g, '-').toLowerCase()}`}>
