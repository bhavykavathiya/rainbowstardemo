...
const BUSINESSES = [
  { t: 'Natural Fancy Color Diamonds', d: 'Earth-mined fancy colors — pinks, blues, yellows, greens — with full GIA grading.', a: '#E5C158' },
  { t: 'Natural White Diamonds', d: 'Colorless D-J range, brilliant precision-cut, all major lab certificates.', a: '#F1D67A' },
  { t: 'CVD Lab-Grown Diamonds', d: 'Type IIa lab-grown stones with IGI/GIA reports — identical brilliance, modern value.', a: '#E5C158' },
...
export const About = () => {
  ...
  return (
    <div data-testid="about-page">
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-[#E5C158]/10">
        <div className="absolute inset-0 bg-gradient-to-br from-[#1F1B12] via-[#14110A] to-[#1F1B12]" />
        <div className="absolute top-0 left-1/3 w-[500px] h-[500px] bg-[#E5C158]/8 rounded-full blur-[150px]"/>
        <div className="relative max-w-7xl mx-auto px-6 py-24">
          <div className="overline mb-4 text-[#E5C158]">About Rainbow Star</div>
          ...
          <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4">
            {STATS.map((s, i) => (
              <motion.div key={i} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{delay: i*0.1}} className="bg-[#1F1B12]/80 border border-[#E5C158]/15 p-5">
                <s.icon size={20} className="text-[#E5C158] mb-3"/>
                <div className="font-serif text-3xl text-[#F1D67A]">{s.n}</div>
                <div className="text-[10px] uppercase tracking-widest text-white/65 mt-1">{s.l}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-6 py-20">
        ...
        <div className="space-y-5 text-white/75 leading-relaxed text-[15px]">
          <p>Since <span className="text-[#E5C158]">2009</span>, Rainbow Star has been actively serving the diamond industry with a commitment to quality, integrity, and long-term relationships.</p>
          <p>Based in <span className="text-[#E5C158]">Surat, Gujarat</span> — the world's leading diamond manufacturing hub — we have established ourselves as a reliable partner in Natural Fancy Color Diamonds, Natural White Diamonds, CVD Lab-Grown Diamonds, and CVD Diamond Manufacturing Services.</p>
          <p>Over the years, our company has developed extensive expertise across sourcing, manufacturing, processing, and supplying diamonds to clients across global markets. Whether serving jewelers, wholesalers, manufacturers, retailers, or investors, our focus remains the same: <span className="text-[#F1D67A]">delivering exceptional value, consistent quality, and dependable service.</span></p>
      ...
      {/* Core businesses */}
      <section className="border-y border-[#E5C158]/10 bg-[#1A160E]/50 py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="overline mb-3 text-[#E5C158]">Core Businesses</div>
          <h2 className="font-serif text-4xl md:text-5xl mb-12">Four pillars of expertise</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {BUSINESSES.map((b, i) => (
              <motion.div key={i} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{delay: i*0.1}} className="bg-[#1F1B12] border border-[#E5C158]/15 p-6 hover:border-[#E5C158]/40 transition">
                <Diamond size={24} style={{color: b.a}} className="mb-4"/>
                <div className="font-serif text-xl mb-2 text-white">{b.t}</div>
                <div className="text-sm text-white/75 leading-relaxed">{b.d}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-24">
        <div className="overline mb-3 text-[#E5C158] text-center">Message From The Chairman</div>
        <h2 className="font-serif text-4xl md:text-5xl mb-12 text-center">Leadership rooted in vision</h2>
        <div className="grid md:grid-cols-5 gap-10 items-start">
          <div className="md:col-span-2">
            <div className="aspect-[3/4] bg-gradient-to-br from-[#2A2418] to-[#14110A] border border-[#E5C158]/20 flex items-center justify-center relative overflow-hidden">
              <div className="absolute inset-0 grain opacity-30"/>
              <div className="text-center relative z-10 px-4">
                <div className="w-32 h-32 mx-auto rounded-full bg-gradient-to-br from-[#E5C158]/30 to-[#B8923A]/10 border-2 border-[#E5C158]/40 flex items-center justify-center mb-5">
                  <span className="font-serif text-5xl text-[#F1D67A]">JK</span>
                </div>
                <div className="font-serif text-2xl text-[#F1D67A]">Jayesh Kavathiya</div>
                <div className="text-xs uppercase tracking-[0.25em] text-white/75 mt-1">Chairman · Rainbow Star</div>
              </div>
            </div>
          </div>
          <div className="md:col-span-3 relative">
            <Quote size={48} className="text-[#E5C158]/20 absolute -top-4 -left-2"/>
            <div className="space-y-5 text-white/75 leading-relaxed text-[15px] relative pl-6 border-l border-[#E5C158]/20">
              ...
              <div className="pt-2">
                <div className="font-serif text-2xl text-[#F1D67A]">— Jayesh Kavathiya</div>
                <div className="text-xs uppercase tracking-[0.2em] text-white/70 mt-1">Chairman, Rainbow Star</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="border-y border-[#E5C158]/10 bg-[#1A160E]/50 py-20">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12">
          <div>
            <div className="overline mb-3 text-[#E5C158]">Our Vision</div>
            <h3 className="font-serif text-3xl md:text-4xl mb-5">A globally respected diamond company.</h3>
            <p className="text-white/75 leading-relaxed">To become a globally respected diamond company recognized for excellence in Natural Diamonds, CVD Lab-Grown Diamonds, and Manufacturing Services while setting benchmarks for quality, trust, innovation, and customer satisfaction.</p>
          </div>
          <div>
            <div className="overline mb-3 text-[#E5C158]">Our Mission</div>
            <h3 className="font-serif text-3xl md:text-4xl mb-5">Six commitments that guide us.</h3>
            <ul className="space-y-3">
              {MISSION.map((m, i) => (
                <li key={i} className="flex gap-3 text-white/75 text-sm leading-relaxed"><CheckCircle2 size={18} className="text-[#E5C158] shrink-0 mt-0.5"/>{m}</li>
              ))}
            </ul>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="overline mb-3 text-[#E5C158]">Why Choose Rainbow Star</div>
        <h2 className="font-serif text-4xl md:text-5xl mb-12">Nine reasons trade partners trust us.</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {WHY.map((w, i) => (
            <motion.div key={i} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{delay: i*0.08}} className="bg-[#1F1B12] border border-[#E5C158]/12 p-6 hover:border-[#E5C158]/35 transition">
              <w.icon size={22} className="text-[#E5C158] mb-3"/>
              <div className="font-serif text-xl mb-1">{w.t}</div>
              <div className="text-sm text-white/65">{w.d}</div>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      {/* CTA Footer */}
      <section className="border-t border-[#E5C158]/10 bg-gradient-to-br from-[#1F1B12] via-[#14110A] to-[#1F1B12] py-20">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <h2 className="font-serif text-4xl md:text-5xl mb-4">Let's build something brilliant together.</h2>
          <p className="text-white/75 mb-8">Trade enquiries, manufacturing partnerships, or bespoke sourcing — our team is one message away.</p>
