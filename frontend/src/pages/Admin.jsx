...
import { Upload, Plus, Trash2, Mail, Image as ImageIcon, FileText, Video, X } from 'lucide-react';
...
const AdminPanel = () => {
  ...
  const loadAll = async () => {
    const [d, e] = await Promise.all([api.get('/diamonds', { params: { limit: 1000 } }), api.get('/enquiries').catch(() => ({ data: [] }))]);
    setStones(d.data); setEnquiries(e.data);
  };
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (editing) await api.put(`/diamonds/${editing}`, payload);
      else await api.post('/diamonds', payload);
  ...
  const del = async (id) => {
    ...
    await api.delete(`/diamonds/${id}`); toast.success('Archived'); loadAll();
  };

  const startEdit = (s) => { setEditing(s.id); setForm(s); window.scrollTo({ top: 0, behavior: 'smooth' }); };

  const upload = async (file, folder, fieldName) => {
    setUploading(u => ({ ...u, [fieldName]: true }));
    try {
      const fd = new FormData(); fd.append('file', file);
      const { data } = await api.post('/uploads', fd, { params: { folder }, headers: { 'Content-Type': 'multipart/form-data' } });
        ...
        <>
          <form onSubmit={save} className="bg-[#1F1B12] border border-white/5 p-6 mb-8" data-testid="admin-form">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
              {[
                { field: 'image_url', folder: 'diamonds', icon: ImageIcon, label: 'Photo', accept: 'image/*' },
