...
const Footer = () => (
  <footer className="border-t border-white/5 bg-[#14110A] mt-24" data-testid="footer">
      ...
      <div>
        ...
        <div className="space-y-2 text-sm">
          {[['/cvd','CVD Diamonds'],['/natural','Natural Diamonds'],['/argyle-pink','Argyle Pink'],['/argyle-blue','Argyle Blue'],['/request','Request a Stone']].map(([to,l]) => (
            <Link key={to} to={to} className="block text-white/75 hover:text-[#E5C158] transition">{l}</Link>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="space-y-2 text-sm text-white/75">
          <div className="flex items-start gap-2"><MapPin size={14} className="mt-1 text-[#E5C158]"/> Surat, Gujarat<br/>India</div>
          <div>{CONTACT.whatsappDisplay}</div>
          <div>{CONTACT.email}</div>
        </div>
      </div>
    </div>
    <div className="border-t border-white/5 py-6 text-center text-xs text-white/75 tracking-wider">
